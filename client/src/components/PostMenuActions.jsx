  ...
  if (!isAuthenticated) {
    return (
      <div className={styles.PostMenuActions}>
        <h2>Actions</h2>
        <p>Please <span style={{ cursor: "pointer", color: "blue" }} onClick={() => navigate("/login")}>log in</span> to perform actions on this post.</p>
      </div>
    );
  }
  ...
  return (
    <div className={styles.PostMenuActions}>
      ...
      {isPending ? (
        ...
        "Fetching Saved Post Failed..."
      ) : (
        <div onClick={handleSaved} style={{ cursor: "pointer" }}>
          <svg
          ...
          {saveMutation.isPending && (
            <span style={{ fontSize: "8px" }}>In Progress...</span>
          )}
        </div>
      )}
      {user?.role === "admin" && (
